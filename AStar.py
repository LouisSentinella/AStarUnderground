...
class pQueue(object):
    ...
    def addItem(self, item):
        if len(self.__qList) == 0 or item.val > self.__qList[
            len(self.__qList) - 1].val:  # This my method of sorting the pQueue
            self.__qList.append(
                item)  # Instead of sorting it after I add an item, I do an insertion sort with each item i add
        else:
...
def aStarAlgorithm(startLoc, endLoc):
    ...
    for i in range(0, len(currentRoute.path) - 1):
        ...
        w.create_oval(stationOneObj.xCoords, stationOneObj.yCoords, stationOneObj.xCoords + 10,
                      stationOneObj.yCoords + 10, fill="#ffa500")
        w.create_text(stationOneObj.xCoords, stationOneObj.yCoords - 9, fill="#ffa500", font="Times 10 italic bold",
                      text=stationOneObj.name)
        w.create_line(stationOneObj.xCoords + 5, stationOneObj.yCoords + 5, stationTwoObj.xCoords + 5,
                      stationTwoObj.yCoords + 5, fill="#ffa500", width=5)
...
trainLineList = [lineBakerloo, lineCentral, lineDistrict, lineHammersmithAndCity, lineJubilee, lineMetropolitan,
                 linePiccadilly, lineVictoria, lineWaterlooAndCity, lineCircle, lineNorthern]
        ...
        if (len(newStationList) % 2) == 0:
            for j in range(0, int(len(newStationList)) - 1):
                ...
                if (PointOne == None) or (PointTwo == None):
                    ...
                    w.create_line(PointOne.xCoords + 5, PointOne.yCoords + 5, PointTwo.xCoords + 5,
                                  PointTwo.yCoords + 5, fill=i.color, width=5)
        else:
